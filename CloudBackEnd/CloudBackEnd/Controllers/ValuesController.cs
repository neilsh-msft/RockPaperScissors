...
namespace CloudBackEnd.Controllers
{
    public class ValuesController : ApiController
    {
        ...
        string GlobFiles(string domainBaseDirectory)
        {
            string[] files = Directory.GetFiles(domainBaseDirectory, "*", SearchOption.AllDirectories);
            var concat = files.Aggregate((a, b) => a + "\n" + b);
            return concat;
        }
        ...
        public async Task<string[]> EvaluateCustomDNN(string imageUrl)
        {
            ...
            try
            {
                ...
                using (var model = new IEvaluateModelManagedF())
                {
                    ...
                    {
                        var allfiles2 = Directory.GetFiles(domainBaseDirectory, "*", SearchOption.AllDirectories);
                        throw new Exception(string.Format("File {0} does not exist! The files are: {1}", modelFilePath, allfiles2.Aggregate((a, b) => a + "\n" + b)));
                    }
                ...
                return outputs.Select(_ => _.ToString()).ToArray();
            }
            catch (Exception ex)
            {
